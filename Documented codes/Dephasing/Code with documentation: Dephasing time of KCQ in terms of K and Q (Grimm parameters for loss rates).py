...
# goal: simulate decay time of the coherent alpha state to get dephasing time (T2) for KCQ
...
def dephasing_time(K_Hz, Q):
    # calculate related system parameters from K and Q
    K = K_Hz * 2 * pi
    alpha_value = alpha(K_Hz * Q)
    e2 = K * alpha_value**2  # calculate the e2 of the optimal initialization
    kappa = omega / Q  # photon loss rate
    kappa_thermal = kappa * 0.08  # photon gein rate
    kappa_eff = 2 * pi * 230  # rate of a^dagger a noise
    ...
    # fit exponential decay (at t=0 the x expectation value of the coherent state is alpha), t is the dephasing time
    def fitted_exponential(x, t):
        return alpha_value * np.exp(-x / t)
...
decay_times = np.zeros((len(K), len(Q)))
for i in range(len(K)):
    for j in range(len(Q)):
        decay_times[i, j] = dephasing_time(K[i], Q[j])
...
contour = plt.contour(
    ...
    Q / 1e6,
    decay_times.T * 1e6,
    levels=np.array([10, 30, 50, 70, 90, 110, 130, 150, 170, 190]),
    cmap="viridis",
)
...
legend_description = Line2D(
    [0], [0], linestyle="", label=r"$\text{Decay times in } \mu s $"
)

# Contour legend labels
legend_labels = ["10", "30", "50", "70", "90", "110", "130", "150", "170", "190"]
legend_colors = [
...
legend_labels.insert(0, r"$\text{Decay times in } \mu s $")
